...
class TransformerDecoderPretrain(L.LightningModule):
    ...
    def __init__(
        ...
        ts_gen_ratio: float = 1.0,  # Ratio for generation involvement [0,1]
    ):
    ...
    def training_step(
        ...
        if use_influence_filtering:
            ...
            if self.hparams.generate_after_epoch <= current_step and random.random() < self.hparams.ts_gen_ratio:
            ...
            if random.random() < self.hparams.ts_gen_ratio and hasattr(self, 'recommended_weights') and self.recommended_weights:
                generated_batch = self._generated_similar_samples(batch)
                batch = self._merge_batches(batch, generated_batch)
                print(f"Step {current_step}: Merged generated samples, new batch size is {len(batch['dataset_index'])}")
    ...
    def _generated_similar_samples(self, batch, num_samples=16):
        ...
        from generation.generate_batch_preview import generate_conditional_batch_decoder, load_model
