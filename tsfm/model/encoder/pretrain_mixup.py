...
class TransformerEncoderPretrain(L.LightningModule):
    ...
    def training_step(
        ...
        # Check for NaN in input batch
        for key, value in batch.items():
            if torch.is_tensor(value) and torch.isnan(value).any():
                print(f"ERROR: NaN detected in input batch['{key}'] at training step {self.global_step}")
        ...
        if use_influence_filtering:
            print(f"Step {current_step}: Using influence-based filtering with mixup augmentation (every {self.hparams.influence_filter_frequency} steps)")
            batch = self._filter_with_high_influence_mixup(
                batch, 
                num_high_influence=16,  # Select top 16 high influence samples for mixup
                final_batch_size=32,   # Final batch size after selection from combined pool
                use_influence_scores=True
            )
        else:
            print(f"Step {current_step}: Using random filtering with mixup augmentation")
            batch = self._filter_with_high_influence_mixup(
                batch, 
                num_high_influence=16,  # Select random 16 samples for mixup
                final_batch_size=32,   # Final batch size after selection from combined pool
                use_influence_scores=False
            )
        
        print(f"Step {current_step}: This batch after filtering and augmentation has {len(batch['dataset_index'])} samples")
    ...
    def on_train_batch_start(self, batch, batch_idx):
        ...
        # print(f"Time taken to before per-example gradients computation: {time.time() - st1} seconds")
        st2 = time.time()
        ...
        # print(f"Time taken to store per-example gradients: {time.time() - st2} seconds")
        st3 = time.time()

        # directly calculate the influence scores on validation gradients
        val_gradients = self.get_validation_gradients_from_trainer()

        print(f"Time taken to get validation gradients: {time.time() - st3} seconds")
        st4 = time.time()
        influence_scores = self.compute_influence_scores_batched(val_gradients)
        print(f"Time taken to compute influence scores: {time.time() - st4} seconds")
        st5 = time.time()
        ...
        print(f"Time taken to after influence score computation: {time.time() - st5} seconds")

    def _filter_with_high_influence_mixup(self, batch, num_high_influence=16, final_batch_size=32, use_influence_scores=True):
        """
        New filtering logic:
        1. Filter 16 high influence samples from the batch
        2. Generate 16 mixup augmented samples from those high influence samples  
        3. Calculate influence scores for the newly generated samples
        4. Choose top 32 samples from original full minibatch (32) + generated samples (16)
        """
        ...
        original_batch_size, seq_len = dataset_indices.shape
        
        # Get unique dataset indices in this batch
        unique_indices = dataset_indices.flatten().unique()
        unique_indices = unique_indices[unique_indices >= 0]  # Remove padding (-1)
        
        if len(unique_indices) < num_high_influence:
            print(f"Batch has only {len(unique_indices)} unique samples, using all samples")
            num_high_influence = len(unique_indices)
        
        # Step 1: Select high influence samples (16) for mixup generation
        if use_influence_scores and self.hparams.enable_influence_scoring:
            ...
            # Sort by influence score (descending) and get the highest scoring samples
            sample_scores.sort(key=lambda x: x[1], reverse=True)
            high_influence_indices = [idx for idx, score in sample_scores[:num_high_influence]]
            
            print(f"Selected top {num_high_influence} high-influence samples for mixup: {high_influence_indices}")
            print(f"Average influence score of selected samples: {sum(score for _, score in sample_scores[:num_high_influence]) / num_high_influence}")
        else:
            # Use random selection
            unique_indices_list = unique_indices.cpu().numpy().tolist()
            high_influence_indices = random.sample(unique_indices_list, num_high_influence)
            
            print(f"Randomly selected {num_high_influence} samples for mixup: {high_influence_indices}")
        
        # Step 2: Extract high influence samples for mixup generation
        high_influence_mask = torch.zeros(original_batch_size, dtype=torch.bool, device=dataset_indices.device)
        for sample_idx in high_influence_indices:
            sample_mask = (dataset_indices == sample_idx).any(dim=1)
            sample_positions = sample_mask.nonzero().squeeze(1)
            if len(sample_positions) > 0:
                high_influence_mask[sample_positions[0]] = True  # Take first occurrence
        
        high_influence_batch = {}
        for key, value in batch.items():
            if torch.is_tensor(value):
                if value.dim() > 0 and value.shape[0] == original_batch_size:
                    high_influence_batch[key] = value[high_influence_mask]
                else:
                    high_influence_batch[key] = value
            else:
                high_influence_batch[key] = value
        
        # Step 3: Generate mixup augmented samples (16) from high influence samples
        augmented_batch = self._create_mixup_augmented_batch(high_influence_batch, num_high_influence)
        
        # Step 4: Combine ORIGINAL FULL BATCH (32) with augmented samples (16)
        # This is the key difference: we keep the full original batch, not just high influence samples
        combined_batch = self._combine_batches(batch, augmented_batch)
        
        # Step 5: Get influence scores for original batch and compute scores for augmented samples only
        if use_influence_scores and self.hparams.enable_influence_scoring:
            # Get influence scores for the original batch
            original_influence_scores = self._get_influence_scores_for_original_batch(batch, dataset_indices)
            
            # Compute influence scores ONLY for the augmented samples (more efficient)
            augmented_influence_scores = self._compute_influence_scores_for_augmented_batch(augmented_batch)
            
            # Combine the scores: original batch scores + augmented batch scores
            combined_influence_scores = torch.cat([original_influence_scores, augmented_influence_scores], dim=0)
        else:
            # For random filtering, assign random scores to the entire combined batch
            combined_batch_size = combined_batch["target"].shape[0]
            combined_influence_scores = torch.rand(combined_batch_size, device=dataset_indices.device)
        
        # Step 6: Select top final_batch_size (16) samples from the combined pool (48)
        top_indices = torch.topk(combined_influence_scores, k=min(final_batch_size, len(combined_influence_scores)), dim=0).indices
        
        final_batch = {}
        for key, value in combined_batch.items():
            if torch.is_tensor(value):
                if value.dim() > 0 and value.shape[0] == combined_batch["target"].shape[0]:
                    final_batch[key] = value[top_indices]
                else:
                    final_batch[key] = value
            else:
                final_batch[key] = value
        
        filter_type = "influence-based" if use_influence_scores else "random"
        print(f"Final batch size ({filter_type} + mixup): {original_batch_size} + {num_high_influence} -> {final_batch['target'].shape[0]}")
        print(f"Combined pool had {len(combined_influence_scores)} samples, selected top {len(top_indices)}")

        # check unique global indices of the final batch
        print(f"Unique global indices of the final batch: {final_batch['dataset_index'].flatten().unique()}")
        print(f"Number of unique augmented samples: {len(final_batch['dataset_index'][final_batch['dataset_index']==-2]) // 512}")
        
        return final_batch

    def _create_mixup_augmented_batch_dummy(self, high_influence_batch, num_augmented_samples):
        return high_influence_batch

    def _create_mixup_augmented_batch(self, high_influence_batch, num_augmented_samples):
        """Create mixup augmented samples from high influence batch using consistent random seeds"""
        
        # Extract the target field for mixup
        target = high_influence_batch["target"]  # Shape: [batch_size, seq_len, patch_size]
        batch_size = target.shape[0]
        
        if batch_size < 2:
            print("Warning: Need at least 2 samples for mixup, returning empty augmented batch")
            return self._create_empty_batch_like(high_influence_batch, 0)
        
        # Pre-generate consistent sample pairs for all fields
        import random
        sample_pairs = []
        for _ in range(num_augmented_samples):
            ts1 = random.randint(0, batch_size - 1)
            ts2 = random.randint(0, batch_size - 1)
            while ts2 == ts1 and batch_size > 1:
                ts2 = random.randint(0, batch_size - 1)
            sample_pairs.append((ts1, ts2))
        
        actual_num_augmented = len(sample_pairs)
        print(f"Using consistent sample pairs for mixup: {sample_pairs}")
        
        # Create augmented batch with same structure as original batch
        augmented_batch = {}
        for key, value in high_influence_batch.items():
            if key == "dataset_index":
                # Create dataset indices for augmented samples
                augmented_batch[key] = torch.full(
                    (actual_num_augmented, value.shape[1]), 
                    -2,  # Special marker for augmented samples
                    dtype=value.dtype, 
                    device=value.device
                )
            elif torch.is_tensor(value) and value.dim() > 0 and value.shape[0] == batch_size:
                # Apply consistent mixup using pre-generated sample pairs
                augmented_values = []
                for ts1, ts2 in sample_pairs:
                    if key in ["target", "label"]:
                        # Apply linear interpolation mixup for continuous data
                        alpha1 = random.random()  # Using same alpha as in mixup function
                        alpha2 = 1 - alpha1
                        augmented_value = alpha1 * value[ts1] + alpha2 * value[ts2]
                    elif key in ["observed_mask", "label_observed_mask"]:
                        # Apply logical OR mixup for boolean masks
                        mask1 = value[ts1].bool()
                        mask2 = value[ts2].bool()
                        augmented_value = torch.logical_or(mask1, mask2).to(value.dtype)
                    else:
                        # For other fields, just take the effective sample (since alpha1=1, alpha2=0)
                        # The effective sample is ts1
                        augmented_value = value[ts1]
                    
                    augmented_values.append(augmented_value)
                
                augmented_batch[key] = torch.stack(augmented_values)
            else:
                # Non-batch tensors or scalars, keep as is
                augmented_batch[key] = value
        
        print(f"Created {actual_num_augmented} mixup augmented samples with consistent sample selection")
        return augmented_batch
    
    def _create_empty_batch_like(self, reference_batch, batch_size):
        """Create an empty batch with the same structure as reference batch"""
        empty_batch = {}
        for key, value in reference_batch.items():
            if torch.is_tensor(value):
                if value.dim() > 0:
                    ...
                    empty_batch[key] = torch.empty(new_shape, dtype=value.dtype, device=value.device)
                else:
    ...
    def _combine_batches(self, batch1, batch2):
        """Combine two batches by concatenating along the batch dimension"""
        combined_batch = {}
        ...
        for key in batch1.keys():
            if key in batch2:
                ...
                if torch.is_tensor(value1) and torch.is_tensor(value2):
                    if value1.dim() > 0 and value2.dim() > 0 and value1.shape[1:] == value2.shape[1:]:
                        # Concatenate along batch dimension
                        combined_batch[key] = torch.cat([value1, value2], dim=0)
                    else:
                        # If shapes don't match or scalars, keep the first batch's value
                        combined_batch[key] = value1
                else:
                    # Non-tensor values, keep the first batch's value
                    combined_batch[key] = value1
            else:
                # Key only in batch1
    ...
    def _get_influence_scores_for_original_batch(self, batch, dataset_indices):
        """Get existing influence scores for the original batch samples"""
        
        batch_size = batch["target"].shape[0]
        influence_scores = torch.zeros(batch_size, device=batch["target"].device)
        
        # Get unique dataset indices in this batch
        unique_indices = dataset_indices.flatten().unique()
        unique_indices = unique_indices[unique_indices >= 0]  # Remove padding (-1)
        
        # For each position in the batch, get the influence score from history
        for i in range(batch_size):
            # Find the dataset index for this batch position
            batch_dataset_indices = dataset_indices[i]
            valid_indices = batch_dataset_indices[batch_dataset_indices >= 0]
            
            if len(valid_indices) > 0:
                dataset_idx = valid_indices[0].item()  # Take the first valid index
                
                # Get influence score from history
                if (hasattr(self, 'influence_score_history') and 
                    dataset_idx in self.influence_score_history):
                    influence_scores[i] = self.influence_score_history[dataset_idx]
                else:
                    influence_scores[i] = 0.0  # Default score if no history
            else:
                influence_scores[i] = 0.0  # Default for padding
        
        print(f"Retrieved influence scores for {batch_size} original samples")
        return influence_scores
    
    def _compute_influence_scores_for_augmented_batch(self, augmented_batch):
        """Compute influence scores ONLY for the augmented samples (more efficient)"""
        
        # Get validation gradients for influence computation
        val_gradients = self.get_validation_gradients_from_trainer()
        # if not val_gradients:
        #     print("Warning: No validation gradients available for augmented samples, using random scores")
        #     batch_size = augmented_batch["target"].shape[0]
        #     return torch.rand(batch_size, device=augmented_batch["target"].device)
        
        # # Check for NaN in validation gradients
        # for name, grad in val_gradients.items():
        #     if torch.isnan(grad).any():
        #         print(f"WARNING: NaN detected in validation gradient for parameter: {name}")
        
        batch_size = augmented_batch["target"].shape[0]
        influence_scores = torch.zeros(batch_size, device=augmented_batch["target"].device)
        
        # Check for NaN in augmented batch
        for key, value in augmented_batch.items():
            if torch.is_tensor(value) and torch.isnan(value).any():
                print(f"WARNING: NaN detected in augmented_batch['{key}']")
        
        # Compute influence scores for each augmented sample
        for i in range(batch_size):
            self.zero_grad()
            # Create a mini-batch with just this augmented sample
            single_sample_batch = {}
            for key, value in augmented_batch.items():
                if torch.is_tensor(value) and value.dim() > 0 and value.shape[0] == batch_size:
                    single_sample_batch[key] = value[i:i+1]
                else:
                    single_sample_batch[key] = value
            
            # Compute gradient for this augmented sample
            try:
                output = self(**{
                    field: single_sample_batch[field] 
                    for field in list(self.train_seq_fields) + ["sample_id"]
                    if field in single_sample_batch
                })
                
                loss = self.hparams.loss_func(
                    pred=output,
                    target=single_sample_batch.get("label"),
                    observed_mask=single_sample_batch.get("label_observed_mask"),
                    prediction_mask=single_sample_batch.get("prediction_mask"),
                    sample_id=single_sample_batch.get("sample_id"),
                    variate_id=single_sample_batch.get("variate_id"),
                )
                
                loss.backward(retain_graph=True)
                
                # Compute influence score as dot product with validation gradients
                influence_score = 0.0
                param_count = 0
                
                for name, param in self.named_parameters():
                    if param.requires_grad and param.grad is not None and name in val_gradients:
                        train_grad = param.grad.flatten()
                        val_grad = val_gradients[name].flatten()
                        
                        # Check for NaN in gradients
                        if torch.isnan(train_grad).any():
                            print(f"WARNING: NaN in training gradient for parameter {name}, sample {i}")
                            continue
                        
                        if train_grad.shape == val_grad.shape:
                            dot_product = torch.dot(train_grad, val_grad).item()
                            if math.isnan(dot_product):
                                print(f"WARNING: NaN in dot product for parameter {name}, sample {i}")
                                continue
                            influence_score += dot_product
                            param_count += 1
                
                influence_scores[i] = influence_score if param_count > 0 else 0.0
                
                # Final check for NaN in influence score
                if math.isnan(influence_scores[i].item()):
                    print(f"WARNING: Final influence score is NaN for sample {i}, setting to 0.0")
                    influence_scores[i] = 0.0
                
            except Exception as e:
                print(f"Error computing influence score for augmented sample {i}: {e}")
                influence_scores[i] = 0.0
        ...
        print("--------------------------------")
        # print("influence_scores: ", influence_scores)
        print(f"Computed influence scores for {batch_size} augmented samples")
        print(f"Augmented score range: {influence_scores.min().item():.4f} to {influence_scores.max().item():.4f}")
        print("--------------------------------")

        # Final check for any NaN in the result
        if torch.isnan(influence_scores).any():
            print("ERROR: NaN values detected in final influence_scores, replacing with zeros")
            influence_scores = torch.where(torch.isnan(influence_scores), torch.zeros_like(influence_scores), influence_scores)
    ...
    def compute_influence_scores_batched(self, val_gradients):
        ...
        try:
            ...
            # Single matrix-vector multiplication: [num_samples]
            ...
            # print range of influence scores
            print(f"Range of influence scores: {torch.min(influence_scores_flat).item()} to {torch.max(influence_scores_flat).item()}")
